// ==============================================
// honey.js — Honey Tree encounter table renderer (BDSP)
//
// Requires (in order):
//   <script crossorigin src="https://unpkg.com/react@18/umd/react.production.min.js"></script>
//   <script crossorigin src="https://unpkg.com/react-dom@18/umd/react-dom.production.min.js"></script>
//   <script src="../javascript/pokemon-core.js"></script>
//   <script src="../javascript/BDSP/honey.js"></script>  (this file)
//
// The data is embedded at the top of this file (window.BDSP_HONEY_TREE_DATA).
//
// Mount selector: .pokemon-bdsp-honey-replace-me
// ==============================================

window.BDSP_HONEY_TREE_DATA = {
  "col1": [
    // slot 0 — 40%
    { "Brilliant Diamond": "Wurmple",  "Shining Pearl": "Wurmple"  },
    // slot 1 — 20%
    { "Brilliant Diamond": "Silcoon",  "Shining Pearl": "Cascoon"  },
    // slot 2 — 20%
    { "Brilliant Diamond": "Combee",   "Shining Pearl": "Combee"   },
    // slot 3 — 10%
    { "Brilliant Diamond": "Burmy",    "Shining Pearl": "Burmy"    },
    // slot 4 — 5%
    { "Brilliant Diamond": "Cherubi",  "Shining Pearl": "Cherubi"  },
    // slot 5 — 5%
    { "Brilliant Diamond": "Aipom",    "Shining Pearl": "Aipom"    }
  ],
  "col2": [
    { "Brilliant Diamond": "Combee",    "Shining Pearl": "Combee"    },
    { "Brilliant Diamond": "Burmy",     "Shining Pearl": "Burmy"     },
    { "Brilliant Diamond": "Cherubi",   "Shining Pearl": "Cherubi"   },
    { "Brilliant Diamond": "Aipom",     "Shining Pearl": "Aipom"     },
    { "Brilliant Diamond": "Heracross", "Shining Pearl": "Heracross" },
    { "Brilliant Diamond": "Wurmple",   "Shining Pearl": "Wurmple"   }
  ]
...
(function () {
    if (!window.React || !window.ReactDOM) { console.warn('[honey-bdsp] React/ReactDOM not found.'); return; }
    if (!window.PokemonCore) { console.warn('[honey-bdsp] pokemon-core.js must be loaded before honey.js.'); return; }
    ...
    // BDSP game names contain spaces; CSS classes use the spaceless lowercase form
    // (e.g. "brilliantdiamond-true"), matching common.css and bdsp.js conventions.
    ...
    function getHoneyTreeData() {
        ...
        // Data is embedded at the top of this file, so HONEY_TREE_DATA is already set.
        if (window.BDSP_HONEY_TREE_DATA) {
            dataPromise = Promise.resolve(window.BDSP_HONEY_TREE_DATA);
            return dataPromise;
        }
        const base = window.HONEY_TREE_BASE || '../javascript/BDSP/';
        dataPromise = loadScript(base + 'honey.js').then(() => {
            if (!window.BDSP_HONEY_TREE_DATA) throw new Error('HONEY_TREE_DATA not found after script load');
            return window.BDSP_HONEY_TREE_DATA;
        });
        return dataPromise;
    }
    ...
    function HoneyFullTable({ data, gameFilter, mount }) {
        ...
        // Each column group spans GAMES.length * 2 cells (sprite + name per game).
        return h('div', { style: { overflowX: 'auto' } },
            h('table', null,
                h('caption', null, caption),
                h('thead', null,
                    h('tr', null,
                        ...
                        ...COL_LABELS.map((label, ci) =>
                            h('th', { key: ci, colSpan: GAMES.length * 2, className: hdrCls(null, base) }, label)
                        ),
    ...
    function HoneyCompressedTable({ data, gameFilter, mount }) {
        ...
        // Combined compressed.
        // Tree headers span GAMES.length columns each.
        return h('div', { style: { overflowX: 'auto' } },
            h('table', null,
                h('caption', null, caption),
                h('thead', null,
                    h('tr', null,
                        ...
                        h('th', { colSpan: GAMES.length, className: hdrCls(null, base) }, 'Normal tree'),
                        h('th', { colSpan: GAMES.length, className: hdrCls(null, base) }, 'Munchlax tree'),
                        h('th', { rowSpan: 2, className: hdrCls(null, base) }, 'Level'),
                    ),
                    h('tr', null,
    ...
    // =========================================================
    // Boot — mount on every .pokemon-bdsp-honey-replace-me node
    // =========================================================
    (function boot() {
        const SELECTOR = '.pokemon-bdsp-honey-replace-me';

        function mountAll() {
            document.querySelectorAll(SELECTOR).forEach(node => {
                if (node.__bdspHoneyMounted) return;
                node.__bdspHoneyMounted = true;
