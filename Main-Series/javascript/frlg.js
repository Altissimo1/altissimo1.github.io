// ==============================================
// frlg.js — Pokémon FireRed / LeafGreen encounter table renderer
//
// Requires (in order):
//   <script crossorigin src="https://unpkg.com/react@18/umd/react.production.min.js"></script>
//   <script crossorigin src="https://unpkg.com/react-dom@18/umd/react-dom.production.min.js"></script>
//   <script src="../javascript/pokemon-core.js"></script>
//   <script src="../javascript/frlg.js"></script>
//
// Structure (mirrors rse.js):
//   - Two games: "FireRed", "LeafGreen"
//   - No conditions (no swarm / Poké Radar / time-of-day)
//   - Per-encounter-type game detection:
//       • Both present → dropdown + 2 columns
//       • FR-only or LG-only → no dropdown, single game-coloured column
//   - Colour prefixes: firered-{true|false}, leafgreen-{true|false}
//   - Encounter types: walking, surfing, fishing, rock_smash
//   - Reads window.FRLG_DATA; location base via window.FRLG_LOCATION_BASE
//   - Mounts on .pokemon-frlg-{walking|surfing|fishing|rocksmash}-replace-me
// ==============================================
(function () {
    if (!window.React || !window.ReactDOM) { console.warn('[frlg] React/ReactDOM globals not found.'); return; }
    if (!window.PokemonCore) { console.warn('[frlg] pokemon-core.js must be loaded before frlg.js.'); return; }
    ...
    // FRLG game constants
    // =========================================================
    const ALL_GAMES = ['FireRed', 'LeafGreen'];
    ...
    // activeGames.length === 1 → single-game (FR- or LG-only): hide game dropdown
    // =========================================================
    ...
    function getLocationData(locId) {
        ...
        const base = window.FRLG_LOCATION_BASE || '../javascript/FRLG/';
    ...
    (function boot() {
        // Walking
        const WALK_SEL = '.pokemon-frlg-walking-replace-me';

        function mountAll() {
            document.querySelectorAll(WALK_SEL).forEach(n => {
                if (n.__frlgWalkMounted) return;
                n.__frlgWalkMounted = true;
        ...
        const SLOT_SELECTORS = [
            { selector: '.pokemon-frlg-surfing-replace-me',   encounterType: 'surfing'    },
            { selector: '.pokemon-frlg-fishing-replace-me',   encounterType: 'fishing'    },
            { selector: '.pokemon-frlg-rocksmash-replace-me', encounterType: 'rock_smash' },
        ];

        for (const { selector, encounterType } of SLOT_SELECTORS) {
            const mountSlotAll = () => {
                document.querySelectorAll(selector).forEach(n => {
                    if (n.__frlgSlotMounted) return;
                    n.__frlgSlotMounted = true;
                    const container = document.createElement('div');
                    for (const a of ['data-location-id', 'data-sprite-base', 'data-sprite-gens']) {
                        const v = n.getAttribute(a);
                        if (v != null) container.setAttribute(a, v);
