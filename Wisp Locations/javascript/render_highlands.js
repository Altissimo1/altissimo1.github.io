...
function updateImage() {
	...
	// Load in the .json with all the spawn data for the Highlands.
	
	fetch('json/highlands.json')
		.then(function (response) {
			return response.json();
		})
		.then(function (data) {
		...
		function appendIcons(data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var cookieName = "highlands" + type + point;
...
function swapIcon(tag) {
	...
	function swap(data, tag) {
		...
		var cookieName = "highlands" + type + tag;
